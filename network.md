...
Claim offer
Based the following, please give me a network diagram, clearlly show firewall rules and access to containers, etc.. # CBC Demo Stack
...
- 2 xWindows 11 with Carbon Black Sensor installed to your registration key and Caldera SandCat sensor.
...
Network Diagram (CBC Demo Stack)
flowchart TB
...
Component	Network	Access Type
Docker Host	192.168.1.37	LAN + Internet
my-net2	Docker bridge	Internal container comms
Windows Containers	my-net2	Internal + outbound internet
Caldera	my-net2	Internal + exposed ports
Kali	my-net2	Internal + SSH exposed
Portainer	Host	External HTTPS
Firewall Rules (Container-Level)
Default Policy
DROP all outbound traffic to LAN (192.168.1.0/24)
ALLOW outbound internet (0.0.0.0/0)
ALLOW internal docker network (my-net2)
Explicit Rules
Block Local Network Access
-A OUTPUT -d 192.168.1.0/24 -j DROP
Allow Docker Internal Communication
-A OUTPUT -d 172.18.0.0/16 -j ACCEPT
Allow Internet (Required for CBC)
-A OUTPUT -p tcp --dport 443 -j ACCEPT
-A OUTPUT -p tcp --dport 80 -j ACCEPT
Restrict to Carbon Black Cloud Only (Optional Tight Mode)
-A OUTPUT -d <CBC_IP_RANGE> -p tcp --dport 443 -j ACCEPT
-A OUTPUT -j DROP
Container Access Matrix
Source	Destination	Port	Purpose	Allowed
User	Portainer	9443/9000	Management UI	Yes
User	Caldera	8888	Web UI	Yes
User	Guacamole	8080/443	VNC Proxy	Yes
User	Kali	2222	SSH	Yes
Guacamole	Windows	VNC (8006)	Remote desktop	Yes
Windows	Caldera	7010+	Agent comms	Yes
Kali	Windows	Any	Attack simulation	Yes
Containers	LAN	Any	Lateral movement	No
Windows	Internet	443	Carbon Black Cloud	Yes
DNS / Service Discovery
caldera-server.my-net2
windows7.my-net2
windows8.my-net2
kali-rolling.my-net2
Docker embedded DNS resolves container names automatically within my-net2.

Key Isolation Controls
No container → LAN access

Only host exposes services externally

All inter-container traffic confined to my-net2

CBC traffic restricted to HTTPS outbound

Optional IP allowlist for Carbon Black Cloud endpoints

Traffic Flow Summary
User connects to host (Portainer, Caldera, Guacamole, Kali).

Guacamole proxies VNC → Windows containers.

Caldera communicates with SandCat agents on Windows.

Kali performs lateral movement within my-net2.

Windows sensors send telemetry → Carbon Black Cloud (HTTPS).

Firewall blocks any attempt to reach local LAN.
